...
public interface IBookRepository extends JpaRepository<Book, Integer> {
    ...
    @Query(value = "select book.*, cart_detail.* from book join cart_detail on cart_detail.book_id = book.id group by book.name order by cart_detail.quantity desc limit 12;", nativeQuery = true)
    List<Book> getBestSeller();

    @Query(value = "select book.* from book where author = :author limit 0, 12;", nativeQuery = true)
    List<Book> getSameAuthor(@Param("author") String author);
}
